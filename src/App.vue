  ...
  <div :class="['fluent-window', isDarkMode ? 'dark-theme' : 'light-theme', { 'is-maximized': isMaximized }]">
    ...
    <main class="fluent-body">
      ...
      <section v-show="currentTab === 'misc'" class="tab-view misc-view animated-view">
        ...
        <!-- 赞助打赏区 (点击图片放大预览) -->
        <div class="fluent-card donate-card">
          <div class="card-header">
            <h3 class="card-title">❤️ {{ t.misc_tab.donate_title }}</h3>
            <span class="card-subtitle">（点击二维码图片可放大查看）</span>
          </div>
          <div class="donate-grid">
            <!-- 微信打赏 -->
            <div class="donate-item wechat-box">
              <div
                class="qr-container zoomable"
                @click="openLightbox('/wechat.png', t.misc_tab.wechat_donate)"
                title="点击放大微信收款码"
              >
                <img src="/wechat.png" alt="微信打赏" class="donate-qr-img" />
                <span class="zoom-badge">🔍 点击放大</span>
              </div>
              <span class="donate-label">{{ t.misc_tab.wechat_donate }}</span>
            </div>

            <!-- USDT 打赏 -->
            <div class="donate-item usdt-box">
              <div
                class="qr-container zoomable"
                @click="openLightbox('/usdt.png', t.misc_tab.usdt_donate)"
                title="点击放大 USDT 收款码"
              >
                <img src="/usdt.png" alt="USDT 打赏" class="donate-qr-img" />
                <span class="zoom-badge">🔍 点击放大</span>
              </div>
              <span class="donate-label">{{ t.misc_tab.usdt_donate }}</span>
              <div class="usdt-address-row" @click="copyUsdtAddress" :title="t.misc_tab.copy_address">
                <span class="usdt-addr-text">{{ usdtAddress }}</span>
        ...
        <!-- 关于我 -->
        <div class="fluent-card about-card">
          <div class="card-header">
            <h3 class="card-title">👤 {{ t.misc_tab.about_title }}</h3>
          </div>
          <div class="about-links">
            <a
              href="javascript:void(0)"
              class="about-link-item"
              @click="openUrl('https://github.com/LinHouYu')"
            >
              <span class="about-icon">🐙</span>
              <span class="about-text">{{ t.misc_tab.github_label }}: https://github.com/LinHouYu</span>
            ...
            <a
              href="javascript:void(0)"
              class="about-link-item"
              @click="openUrl('https://space.bilibili.com/1563740453')"
            >
              <span class="about-icon">📺</span>
              <span class="about-text">{{ t.misc_tab.bilibili_label }}: https://space.bilibili.com/1563740453</span>
              <span class="external-icon">↗</span>
            </a>
          </div>
        </div>
    ...
    <!-- 图片放大预览 Lightbox 模态框 -->
    <div v-if="lightboxData" class="image-lightbox-overlay" @click="lightboxData = null">
      <div class="lightbox-container" @click.stop>
        <div class="lightbox-header">
          <span class="lightbox-title">{{ lightboxData.title }}</span>
          <button class="lightbox-close-btn" @click="lightboxData = null" title="关闭">✕</button>
        </div>
        <div class="lightbox-img-wrapper">
          <img :src="lightboxData.src" :alt="lightboxData.title" class="lightbox-large-img" />
        </div>
        <div class="lightbox-footer">
          <span>（按 ESC 键或点击外部可关闭）</span>
        </div>
      </div>
    </div>
...
const fixedTunnelNodeCodes = ref<string[]>(JSON.parse(localStorage.getItem('fixed_tunnel_node_codes') || '[]'));
const isFetchingTempUrl = ref(false);

// 服务端运行状态文本计算 (免配置临时通道显示 1 个获取到的节点，专属固定通道显示最多 3 个高可用边缘节点)
const serverStatusText = computed(() => {
  ...
  // 1. 免配置临时通道 (显示单个获取到的节点)
  if (serverMode.value === 'temp') {
    const baseStatus = tempTunnelUrl.value
  ...
  // 2. 专属固定通道 (显示最多 3 个高可用边缘节点)
  const baseStatus = t.value.server_tab.status_running;
  if (fixedTunnelNodeCodes.value.length > 0) {
    const formattedNodes = fixedTunnelNodeCodes.value.slice(0, 3).map((code) => {
      const codeKey = code.toLowerCase();
      const codeUpper = code.toUpperCase();
      const region = nodesMap[codeKey];
      return region ? `${codeUpper} - ${region}` : codeUpper;
    });
    return `${baseStatus} (${nodeLabel}: ${formattedNodes.join(', ')})`;
  }
...
watch(
  fixedTunnelNodeCodes,
  (newVal) => {
    if (newVal && newVal.length > 0) {
      localStorage.setItem('fixed_tunnel_node_codes', JSON.stringify(newVal));
    } else {
      localStorage.removeItem('fixed_tunnel_node_codes');
    }
  },
  { deep: true }
);
...
const lightboxData = ref<{ src: string; title: string } | null>(null);
const toastMessage = ref('');
const consoleBodyRef = ref<HTMLDivElement | null>(null);

// USDT 打赏地址
const usdtAddress = 'TQhtmLB9A7xjjPzXJZv95g7XzRo5QhXFVq';
...
const logs = ref<LogEntry[]>([
  {
    ...
    message: 'Cloudflare Tunnel GUI (Windows 11 Fluent) 已启动就绪',
    level: 'info',
    source: 'system',
  },
]);
...
const copyUsdtAddress = async () => {
  try {
    ...
    showToast(t.value.misc_tab.copy_success);
    appendLog(`[INFO] 已复制 USDT 地址到剪贴板: ${usdtAddress}`, 'info', 'misc');
  } catch {
    appendLog('复制 USDT 地址失败', 'error');
  }
};
  ...
  try {
    await listen<{ message: string; level: LogEntry['level']; source: LogEntry['source'] }>(
      ...
      (event) => {
        ...
        // 自动抓取 trycloudflare.com 临时公网域名与 Cloudflare 节点代码 (如 location=eze01, location=sjc)
        if (event.payload.source === 'server' || event.payload.source === 'system') {
          ...
          // 正则提取 Cloudflare 节点代码（忽略末尾数字，如 eze01 -> eze）
          const locMatch = event.payload.message.match(/location[=:\s"]+([a-zA-Z]+)\d*/i)
            || event.payload.message.match(/location=([a-zA-Z0-9]+)/i);
          if (locMatch && locMatch[1]) {
            ...
            if (rawLetters.length === 3) {
              if (serverMode.value === 'temp') {
                ...
                // 专属固定通道最多收集并展示 3 个不同边缘节点
                if (!fixedTunnelNodeCodes.value.includes(rawLetters) && fixedTunnelNodeCodes.value.length < 3) {
                  fixedTunnelNodeCodes.value.push(rawLetters);
                }
              }
            }
          }
...
.qr-container.zoomable {
  cursor: zoom-in;
  transition: transform 0.2s ease, box-shadow 0.2s ease;
}

.qr-container.zoomable:hover {
...
.qr-container.zoomable:hover .zoom-badge {
  opacity: 1;
}
...
.lightbox-container {
  ...
  max-height: 90vh;
  cursor: default;
  animation: modalPop 0.22s cubic-bezier(0.1, 0.9, 0.2, 1);
}
