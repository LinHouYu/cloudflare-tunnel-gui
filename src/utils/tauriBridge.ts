...
export async function safeInvoke<T>(cmd: string, args?: Record<string, unknown>): Promise<T> {
  ...
  switch (cmd) {
    ...
    case 'start_server_tunnel': {
      ...
      setTimeout(() => {
        emitMockLog(`[INFO] Registered tunnel connection connIndex=0 connection=cf-${Math.random().toString(16).substr(2, 8)} location=hkg01 protocol=quic`, 'info', 'server');
        emitMockLog(`[INFO] Registered tunnel connection connIndex=1 connection=cf-${Math.random().toString(16).substr(2, 8)} location=nrt02 protocol=quic`, 'info', 'server');
        emitMockLog(`[INFO] Registered tunnel connection connIndex=2 connection=cf-${Math.random().toString(16).substr(2, 8)} location=sjc01 protocol=quic`, 'info', 'server');
        emitMockLog(`[INFO] 已在本地 127.0.0.1:${port} 建立入口代理服务`, 'info', 'server');
        emitMockLog(`[SUCCESS] 隧道 [${name}] 已成功连接至 3 个高可用边缘路由节点 (HKG, NRT, SJC)`, 'success', 'server');
      }, 500);
